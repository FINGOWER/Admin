...
var price_settings = function (callback) {
  config.load(function (err, config) {
    ...
    callback(null, {
      provider: config.exchanges.plugins.current.BTC.ticker,
...
var wallet_settings = function (callback) {
  config.load(function (err, config) {
    ...
    var provider = config.exchanges.plugins.current.BTC.transfer
...
var exchange_settings = function (callback) {
  config.load(function (err, config) {
    ...
    var provider = config.exchanges.plugins.current.BTC.trade
    if (!provider) {
  ...
  return {
    ...
    user: function () { // grabs all price/wallet/exhange data
      async.parallel({
        ...
        if (err) return res(err)  // if err don't try to return data
